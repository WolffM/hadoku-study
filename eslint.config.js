  ...
  // -------------------------------------------------------------
  {
    ...
    languageOptions: {
      ...
      parserOptions: {
        ...
        project: [
          './tsconfig.json',
          ...(existsSync('./worker/tsconfig.json') ? ['./worker/tsconfig.json'] : [])
        ]
      },
      globals: {
    ...
    rules: {
      ...
      // TS resolves identifiers itself (incl. type-only names like React in
      // React.PointerEvent); no-undef on TS files only false-positives.
      'no-undef': 'off',

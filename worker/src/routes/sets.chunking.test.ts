/**
 * The card INSERT has to fit inside D1's bound-parameter ceiling.
 *
...
import { CARD_COLUMNS, D1_MAX_BOUND_PARAMS, INSERT_CHUNK } from './sets.js';

describe('card insert chunking', () => {
	it('never binds more parameters than D1 accepts', () => {
		expect(INSERT_CHUNK * CARD_COLUMNS).toBeLessThanOrEqual(D1_MAX_BOUND_PARAMS);
	});
	...
	it('uses the ceiling it is given, rather than being needlessly small', () => {
		...
		expect((INSERT_CHUNK + 1) * CARD_COLUMNS).toBeGreaterThan(D1_MAX_BOUND_PARAMS);
	});

	it('still writes a full-size set in a sane number of statements', () => {
		// 500 cards is MAX_CARDS_PER_SET. One statement per card would put the
		// batch's size in the hands of whoever pasted the deck.
	...
	it('holds for the board that found the bug', () => {
		...
		// shape that 500d in production on 2026-08-21.
		const chunks = Math.ceil(25 / INSERT_CHUNK);
		expect(chunks).toBeGreaterThan(1);
		for (let i = 0; i < chunks; i += 1) {
			const rows = Math.min(INSERT_CHUNK, 25 - i * INSERT_CHUNK);
			expect(rows * CARD_COLUMNS).toBeLessThanOrEqual(D1_MAX_BOUND_PARAMS);
		}
	});
});
